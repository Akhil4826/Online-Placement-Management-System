                    ...
                    <div class="panel panel-default table-responsive">
                        <table class="table table-striped table-bordered templatemo-user-table">
                            ...
                            <tbody>
                                ...
                                $num_rec_per_page = 15;
                                $mysqli = new mysqli('localhost', 'root', 'root@123', 'placement');
                                ...
                                $sql = "SELECT * FROM basicdetails WHERE Approve = 0 AND Branch = ? LIMIT ?, ?";
                                ...
                                // Pagination
                                $sql = "SELECT COUNT(*) FROM basicdetails WHERE Approve = 0 AND Branch = ?";
                                $stmt = $mysqli->prepare($sql);
                                $stmt->bind_param("s", $p);
                                $stmt->execute();
                                ...
                                if ($totalpage > 1) {
                                    ...
                                    if ($currentpage > 1) {
                                        $prev = $currentpage - 1;
                                        echo "<li><a href='manage-student.php?page=$prev&branch=$p'>&lt;</a></li>";
                                    }

                                    // Page numbers
                                    for ($i = max(1, $currentpage - 2); $i <= min($totalpage, $currentpage + 2); $i++) {
                                        echo "<li><a href='manage-student.php?page=$i&branch=$p'>$i</a></li>";
                                    }

                                    // Next button
                                    if ($currentpage < $totalpage) {
                                        $nxt = $currentpage + 1;
                                        echo "<li><a href='manage-student.php?page=$nxt&branch=$p'>&gt;</a></li>";
                                    }
